...
- **Perf: `add_memory` post-write read-back now uses a lightweight existence probe.** New `MEMORY_EXISTS_BY_ID` query (`SELECT id` only) + `PostgresDatabase.memory_exists()` + `MemorySystem.memory_exists()` wrapper (getattr-guarded, falls back to `get_memory` for backends/mocks without it). The previous read-back fetched and parsed the entire embedding vector on EVERY write just to confirm persistence — measurable added latency on large memories.
- **Perf: auto-mode RRF embeds the question exactly once and fans out concurrently.** `_query_multi_model_rrf` passed its precomputed 384-dim vector into `vector_only_search(query_vector=...)` (previously the question was embedded twice per query), and the 384/1024 fan-out arms now run under `asyncio.gather` instead of sequential awaits — matching what the docstring always claimed.
- **Added: `MEMINI_OPERATION_TIMEOUT_MS` config field (default `30000`, clamped to [1000, 600000]).** The per-MCP-tool-call timeout was hard-coded at 30s (`OPERATION_TIMEOUT` in server.py); large-memory writes on slow CPU embedding backends (e.g. BGE-M3 on long text) could exceed it and surface as MCP `-32001` timeouts. All ~105 `asyncio.wait_for` call sites now resolve via `_op_timeout()`. Raise the env var for slow hardware; lower it to fail fast.
- **Chore: fixed 15 latent mypy errors** surfaced by a mypy upgrade (all type-only, zero behavior change): pgembed optional-import ignores in `driver.py`/`cli.py`, first-binding union annotations for the installer team-connection prompt vars, `RRFDatabase` wrap annotation in `create_database`, typed returns on the image-arm search path, `memini_vision` lazy-import ignore, readback exception var declaration in `server.py`.
- **Tests**: 17 new regression tests in `tests/test_v156_perf.py` covering all four changes (SQL projection assertions, missing-embedding-key tolerance, single-embed RRF counting, precomputed-vector passthrough, timeout clamp/env resolution, read-back probe preference, and a source-level guard that no call site pins the constant directly). Suite: **1095 passing, 0 failed**, 56 skipped. Installed the optional `[vision]` extra (`memini-vision==0.1.1`) so the 13 image-RRF tests run again — they were failing on ModuleNotFoundError before this release. `ruff check` 0 errors. `mypy src/` 0 errors (the prior 14 pre-existing errors are fixed).
